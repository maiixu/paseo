...
const input = {
  ...
  profiles: [profile],
  initialValues: { serverId: "cloud", workingDir: "/project" },
...
describe("pinned launch profile", () => {
  ...
  it("uses the selected host's profile values without changing its explicit host", () => {
    const selected = resolvePinnedLaunchProfile({
      ...input,
      initialValues: { serverId: "mac", workingDir: "/mac-project" },
