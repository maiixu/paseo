...
export function resolveLaunchSnapshotServerId(input: {
  profileId: string | null;
  ...
  if (!input.profileId || input.userModifiedServerId) return input.currentServerId;
  return input.initialValues?.serverId ?? input.initialServerId ?? input.currentServerId;
}
